...
/**
 * Subscribe to the thread set for the active note.
 *
 * Fetches via slice 2.1's `listThreadsForNote` on note change and
 * re-fetches whenever `tolaria:thread-created` (new thread filed)
 * or `tolaria:thread-updated` (reply added or status changed) fires
 * — so any mutation routed through the window-event seam shows up
...
export function useThreadsForNote({
  ...
  useEffect(() => {
    ...
    window.addEventListener(THREAD_UPDATED_EVENT, handler)

    return () => {
      cancelled = true
      window.removeEventListener(THREAD_CREATED_EVENT, handler)
      window.removeEventListener(THREAD_UPDATED_EVENT, handler)
    }
